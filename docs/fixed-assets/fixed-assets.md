...
- **الإدارة المُحكمة**: تنظيم وتصنيف الأصول بمرونة لتلبية احتياجات عملك الفريدة
...
- يتم تحديد **العمر الافتراضي** للأصل و**تاريخ بداية الإهلاك** عند الشراء
- تتوفر أيضًا **طلب شراء أصل** و**أمر شراء أصل**
...
**الوظيفة**: هو المستند الذي يسجل استهلاك قيمة الأصل على مدى عمره الافتراضي.
...
**الغرض**: لتسجيل أي تغيير في قيمة الأصل أو عمره الافتراضي، مثل أعمال **العمرة الكبرى** التي تزيد من قيمته أو عمره، أو بيع جزء منه.
...
**الوظيفة**: يستخدم لبيع الأصل أو إزالته من سجلات الشركة.
...
**الغرض**: يُستخدم لإدخال بيانات الأصول الموجودة مسبقًا في أنظمة سابقة أو سجلات يدوية عند بدء استخدام النظام الجديد.

### البيانات المطلوبة
يتم إدخال معلومات مثل:
- **تاريخ بداية الإهلاك**
- **العمر الافتراضي**
- **العمر المتبقي**
- **قيمة الاقتناء الافتتاحية**
- **الإهلاك التراكمي الافتتاحي**

> **ملاحظة**: يوجد مستند خاص لتعديل هذه البيانات الافتتاحية في حال وجود أخطاء.
...
**الاستخدام**: يُستخدم للأصول التي تسجل بالكمية (مثل عدد كراسي)، ويسمح بالتخلص من عدد معين من هذه الكمية دون التخلص من الأصل بأكمله.
...
**الوظيفة**: يستخدم لتسجيل استلام أصل **تحت الفحص** قبل قبوله بشكل نهائي.


## صيانة الأصول الثابتة

نظام صيانة الأصول هنا **أبسط** من وحدات الصيانة المتخصصة الأخرى في النظام مثل **صيانة المعدات** و**صيانة السيارات** التي تحتوي على تفاصيل وحالات عمل أكثر تعقيدًا.
...
- عند تنفيذ الصيانة، يتم إنشاء سجل صيانة عن طريق اختيار **خطة الصيانة**، **الأصل**، و**نوع الصيانة**، مع إضافة بيانات مثل **المهندس المسؤول**
...
## اعتمادات الأصول (Asset Letters of Credit)
...
### المصروفات الإضافية
تشمل هذه الاعتمادات مصاريف إضافية مثل:
- **الجمارك**
- **الضرائب**
- **الأرضيات**
- **النقل** (بحري وجوي)
...
- **سند تكاليف اعتمادات أصول**: بعد إغلاق الاعتماد، يتم إنشاء هذا السند الذي يجمع التكاليف، ينشئ الأصول، ويبدأ احتساب الإهلاك من تاريخ تكليف الاعتماد

### الفرق بين اعتمادات الأصول والاعتمادات المستندية
...
- رغم تشابه دورة العمل، فإن **الهدف النهائي** و**التأثير المحاسبي** يختلف
## جرد الأصول (Asset Inventory)

**الهدف**: يهدف إلى تحديد الأصول الموجودة في أماكن معينة.

- يساعد في الكشف عن **الأصول الناقصة** أو **الزائدة** للتحقيق في أسباب الفروقات

## نقل الأصول (Asset Transfer)

**الاستخدام**: يُستخدم لنقل أصل من شركة لأخرى أو من مكان لآخر.

- يوجد **طلبات فردية** و**طلبات مجمعة** لنقل الأصول

## خروج الأصول (Asset Out)

**الوظيفة**: يمكن إخراج الأصل من حيازة الشركة **مؤقتًا** لأغراض مثل الصيانة، مع تحديد **تاريخ الإعادة المتوقع**.
...
- **التخلص من العهدة**: يتم تسجيل التخلص من العهدة (إذا تعطلت، بيعت، احترقت)

### قيمة التخلص
- يمكن تحديد **قيمة التخلص**
- إذا كانت **صفر**، فهذا يعني التخلص منها دون عائد
- إذا كانت لها قيمة، فغالبًا ما تُسجل كـ**إيرادات أخرى**

> **ملاحظة**: هذه العهد **لا تخضع للإهلاك**.

تغطي هذه العمليات الشاملة إدارة الأصول الثابتة من **الاقتناء** وحتى **التخلص** منها، بما في ذلك **الصيانة** و**التتبع الدقيق**.
